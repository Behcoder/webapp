...
class _ConnectivityWrapperState extends State<ConnectivityWrapper> {
  ...
  @override
  Widget build(BuildContext context) {
    // اگر در حال بارگذاری است، صفحه بارگذاری نمایش بده
    if (_isLoading) {
      return Scaffold(
        body: Center(
          child: Column(
    ...
    // اگر خطایی وجود دارد، صفحه خطا نمایش بده
    ...
    // اگر اتصال اینترنت وجود ندارد، صفحه عدم اتصال نمایش بده
    if (!_isConnected) {
      return const NoInternetPage();
    }

    // اگر اتصال اینترنت وجود دارد، صفحه اصلی نمایش بده
...
class _ProductDisplayWidgetState extends State<ProductDisplayWidget>
  ...
  final Map<String, Color> _categoryColors = {
    'pipes': const Color(0xFFD6EAF8),
    'fittings': const Color(0xFFD5F5E3),
    'valves': const Color(0xFFFADBD8),
    'uncategorized': const Color(0xFFE9ECEF),
  };

  final Map<String, Color> _categoryTextColors = {
    'pipes': const Color(0xFF154360),
    'fittings': const Color(0xFF145A32),
    'valves': const Color(0xFF7B241C),
    'uncategorized': const Color(0xFF343A40),
  };
  ...
  Future<void> _fetchProducts() async {
    try {
      final url = Uri.parse(
          'https://seify.ir/wp-json/wc/v3/products?consumer_key=ck_278d4ac63be04448206d8aec329301bd58831670&consumer_secret=cs_c4d143188011db4cce3137dd7c046c435f18114b&per_page=100');
      ...
      if (response.statusCode == 200) {
        ...
        // Filter products that have prices and convert to our format
        ...
        // Shuffle the products for random selection
        ...
        setState(() {
          _isLoading = false;
          _errorMessage = 'خطا در دریافت محصولات: ${response.statusCode}';
        });
      }
    } catch (e) {
      setState(() {
  ...
  void _startProductRotation() {
    ...
    // Refresh products every 30 seconds to get new random products
  ...
  Color _getGradientColor(double value) {
    ...
    final progress = (value * (colors.length - 1)) - index;
    
    return Color.lerp(colors[index], colors[nextIndex], progress)!;
  }

  @override
  Widget build(BuildContext context) {
    return Container(
      height: MediaQuery.of(context).size.height * 0.35, // Increased from 0.2 to 0.35
      width: double.infinity,
  ...
  Widget _buildContent() {
    ...
    final currentProduct = _products[_currentProductIndex];
          ...
          child: Container(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [
                  _getGradientColor(_gradientController.value),
                  _getGradientColor((_gradientController.value + 0.25) % 1.0),
                  _getGradientColor((_gradientController.value + 0.5) % 1.0),
                  _getGradientColor((_gradientController.value + 0.75) % 1.0),
                ],
              ),
              borderRadius: BorderRadius.circular(8),
              boxShadow: [
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _categoryColors[currentProduct['category']]?.withOpacity(0.85),
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.bold,
                          color: _categoryTextColors[currentProduct['category']],
                        ),
                        textAlign: TextAlign.center,
                      ),
                    ),
                    ...
                    AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: _buildPriceDigits(currentProduct['price']),
                        );
                      },
                    ),
                  ],
  ...
  List<Widget> _buildPriceDigits(String price) {
    final digits = price.replaceAll(',', '').split('');
          ...
          child: Center(
            child: AnimatedBuilder(
              ...
              builder: (context, child) {
                ...
                final animatedValue = _priceController.value;
                final currentDigit = (digit + (animatedValue * 10).round()) % 10;
